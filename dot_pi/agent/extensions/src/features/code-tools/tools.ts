...
export function registerCodeTools(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    label: "Code Overview",
    description:
      "Return compact git-aware codebase overview. Use before ls/find for unfamiliar repos.",
    promptSnippet:
      "Use code-overview for first-pass repo exploration instead of raw ls/find.",
    parameters: Type.Object({
      path: Type.Optional(
        Type.String({ description: "Directory to inspect. Defaults to cwd." }),
      ),
      limit: Type.Optional(
        Type.Integer({
          minimum: 1,
          maximum: 80,
          description: "Max git status rows. Default 40.",
        }),
      ),
    }),
    execute: async (_toolCallId, params: OverviewParams) => {
      ...
      return {
        ...
        details: { root, cwd, files: files.length, status: status.length },
      };
    },
    renderCall(args, theme) {
      return renderToolCall("code-overview", args.path ?? ".", theme);
    },
  ...
  pi.registerTool({
    ...
    label: "Code Search",
    description:
      "Ripgrep-backed code search with capped ranked snippets. Use instead of broad grep.",
    promptSnippet:
      "Use code-search for repository content lookup instead of broad grep.",
    promptGuidelines: [
  ...
  pi.registerTool({
    ...
    label: "Code Files",
    description:
      "List file paths by glob or type. fd-backed with rg/git fallbacks. Use instead of ls/find.",
    promptSnippet:
      "Use code-files for file path listing instead of bash ls or find.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      ...
      glob: Type.Optional(
        Type.String({ description: "Glob filter, e.g. **/*.ts or *.nu." }),
      ),
      type: Type.Optional(
        Type.String({
          description: "Filter by 'file' or 'dir'. Omit for both.",
        }),
      ),
      limit: Type.Optional(
        Type.Integer({
          minimum: 1,
          maximum: MAX_FILES_RESULTS,
    ...
    execute: async (_toolCallId, params: FilesParams) => {
      ...
      const typeFilter =
        params.type === "file" || params.type === "dir"
          ? params.type
          : undefined;
      const { files, truncated, via } = await listFiles(
        pi,
        searchDir,
        params.glob,
        typeFilter,
    ...
    renderCall(args, theme) {
      return renderToolCall(
        "code-files",
        [args.path ?? ".", args.glob, args.type].filter(Boolean).join(" · "),
        theme,
      );
    },
    renderResult(result, _, theme) {
