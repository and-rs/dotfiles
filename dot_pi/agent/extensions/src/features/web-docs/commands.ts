...
export function registerWebDocsCommands(pi: ExtensionAPI): void {
  pi.registerCommand("exa", {
    ...
    handler: async (args, ctx) => {
      ...
      if (!action) {
        const choice = await ctx.ui.select("Exa", [
          "login",
          "status",
          "logout",
          "service-login",
      ...
      if (action === "service-login") {
        ...
        if (!value?.trim())
          return ctx.ui.notify("No service key saved", "info");
      ...
      if (action === "status") {
        ...
        if (!resolved.key || !resolved.source)
          return ctx.ui.notify("Exa not configured", "warning");
        return ctx.ui.notify(
          `Exa configured via ${formatExaSource("api", resolved.source)}`,
          "info",
        );
      }
      if (action === "service-status") {
        ...
        if (!resolved.key || !resolved.source)
          return ctx.ui.notify("Exa service key not configured", "warning");
        return ctx.ui.notify(
          `Exa service key configured via ${formatExaSource("service", resolved.source)}`,
          "info",
        );
      }
      if (action === "usage") {
        ...
        if (!resolved.key)
          return ctx.ui.notify(
            "Exa service key not configured. Use /exa service-login.",
            "warning",
          );
        try {
          return ctx.ui.notify(
            await computeExaUsageSummary(resolved.key),
            "info",
          );
        } catch (error) {
