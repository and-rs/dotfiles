...
export function parseHtmlDocument(
  html: string,
  url: string,
): { title: string | null; markdown: string; text: string; html: string } {
  ...
  const bodyHtml =
    dom.window.document.body?.innerHTML?.trim() || sourceHtml || html;
  const bodyText = normalizeWhitespace(
    dom.window.document.body?.textContent?.trim() || sourceText,
  );
