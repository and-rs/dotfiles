...
#
# Select specific header interactively
def qsv-select [] { let has_stdin = (
  $in != null and $in | into string | str trim | is-not-empty
)
let file = if $has_stdin {
  null
} else {
  let picked = (^fd -t f -e csv -e tsv -e ssv -e tab -H --follow --color=never --exclude .git | ^fzf --prompt="File > " --layout=reverse --height=40%)
  if ($picked | is-empty) {
