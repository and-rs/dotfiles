alias ai = aichat -r meaningful
alias ais = aichat -r meaningful -s
alias aie = aichat -e
alias "ai gc" = aichat -r meaningful --macro commit-staged
...
def ret [
  ...
  let raw = (
    yek ...$yek_args --json | from json | each {|r| {path: $r.filename contents: $r.content} } | if ($glob | is-not-empty) and not $select {
