...
export default function registerCodeViewFeature(pi: ExtensionAPI): void {
  pi.registerTool({
    ...
    label: "Code View",
    description:
      "Read a bounded source range with plain line numbers. Does not edit files.",
    promptSnippet:
      "Use code-view to inspect exact source after discovery. Cite its path and line numbers when explaining behavior.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      ...
      start: Type.Optional(
        ...
        Type.Integer({
          minimum: 1,
          description: "Last line to read. Defaults to 400 lines after start.",
        }),
      ),
    }),
    execute: async (
      _toolCallId,
      params: CodeViewParams,
      _signal,
      _onUpdate,
      ctx,
    ) => {
      const result = await readCodeView(
        ctx.cwd,
        params.path,
        params.start,
        params.end,
      ...
      return {
        content: [{ type: "text", text: formatCodeView(result) }],
        details: result,
      };
    },
    renderCall(args, theme) {
      const range =
        args.start === undefined
          ? ""
          : `:${args.start}${args.end === undefined ? "" : `-${args.end}`}`;
      return new Text(
        `${theme.fg("toolTitle", theme.bold("code-view"))} ${theme.fg("accent", `${args.path ?? ""}${range}`)}`,
        0,
        0,
      );
    },
    renderResult(result, _, theme) {
      ...
      if (!details)
        return new Text(theme.fg("warning", "No source details"), 0, 0);
