...
import { CUSTOM_TYPE, MIN_MASK_CHARACTERS, PRESERVED_EDIT_RESULTS, PRESERVED_HASHLINE_READS, RAW_RECENT_USER_TURNS, type ContextMessage, type MaskStats, type MessageContent, type TextContent, type ToolCallContent, type ToolCallInfo } from "./types.ts";
import { normalizeToolName, summarizeToolResult, textFromContent } from "./summaries.ts";
...
function shouldMask(message: ContextMessage, index: number, rawWindowStart: number, preservedIds: Set<string>): boolean {
  ...
  return !text.startsWith("[context masked:") && text.length >= MIN_MASK_CHARACTERS;
}

export function maskContext(messages: CoreAgentMessage[]): { messages: CoreAgentMessage[]; stats: MaskStats } {
  ...
  const stats: MaskStats = { maskedCount: 0, beforeCharacters: 0, afterCharacters: 0, tools: new Map(), ids: [], samples: [] };
  const nextMessages = sourceMessages.map((message, index) => {
    ...
    if (stats.samples.length < 6) stats.samples.push(`${toolName} ${beforeText.length}→${afterText.length}`);
    return {
      ...message,
