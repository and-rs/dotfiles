...
export default function registerQuickfixHandoffFeature(pi: ExtensionAPI): void {
  ...
  pi.registerCommand("qf", {
    ...
    handler: async (_args, ctx) => {
      if (!lastQuickfixScript) return ctx.ui.notify("No quickfix handoff in this Pi session.", "warning");
      try {
  ...
  pi.registerTool({
    ...
    description: "Validate repository locations and render a user-run Nushell command that opens them in Neovim quickfix.",
    promptSnippet: "Whenever a repository reply points user to verified source, use quickfix-handoff to render the user-run command.",
    promptGuidelines: [
    ...
    parameters: Type.Object({
      locations: Type.Array(
        Type.Object({
          ...
          line: Type.Integer({ minimum: 1, description: "Verified 1-based line number." }),
          column: Type.Optional(Type.Integer({ minimum: 1, description: "1-based column. Default 1." })),
          reason: Type.String({ minLength: 1, description: "One-line reason for opening this location." }),
        }),
        { minItems: 1, maxItems: 50, description: "Locations to include in quickfix order." },
      ),
    }),
    execute: async (_toolCallId, params: QuickfixHandoffParams, _signal, _onUpdate, ctx) => {
      const handoff = await createQuickfixHandoff(ctx.cwd, params.locations);
      lastQuickfixScript = handoff.script;
      return { content: [{ type: "text", text: handoff.script }], details: handoff };
    },
    renderCall(_args, theme) {
      return new Text(theme.fg("toolTitle", theme.bold("quickfix-handoff")), 0, 0);
    },
    renderResult(result, { expanded }, theme) {
      const handoff = result.details as QuickfixHandoff | undefined;
      if (!handoff) return new Text(theme.fg("warning", "No quickfix handoff"), 0, 0);
      const script = result.content.find((item) => item.type === "text")?.text ?? "";
      return new Text(`\n${theme.fg("toolOutput", script)}`, 0, 0);
    },
  });
}
