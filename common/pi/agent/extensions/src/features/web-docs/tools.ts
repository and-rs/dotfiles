   ...
   return [
      defineTool({
         ...
         parameters: Type.Object({
            query: Type.String({
               description:
                  "Search query for the docs, API, error, or concept.",
            }),
            type: Type.Optional(
               Type.Union(
                  SEARCH_TYPES.map((value) => Type.Literal(value)),
                  { description: "Search depth and latency tradeoff." },
               ),
            ),
            ...
            includeDomains: Type.Optional(
               Type.Array(
                  Type.String({
                     description: "Domain to prefer, for example docs.exa.ai",
                  }),
                  {
                     maxItems: 10,
         ...
         execute: async (_toolCallId, params) => {
            ...
            if (!resolved.key)
               throw new Error(
                  "No Exa API key. Use /exa login or set EXA_API_KEY.",
               );
         ...
         renderResult(result, { expanded, isPartial }, theme) {
            ...
            const details = result.details as
               | { results?: ExaSearchResult[]; type?: SearchType }
               | undefined;
      ...
      defineTool({
         ...
         parameters: Type.Object({
            ...
            mode: Type.Optional(
               Type.Union(
                  FETCH_MODES.map((value) => Type.Literal(value)),
                  {
                     description:
                        "Returned format: markdown, text, or html. Default markdown.",
                  },
               ),
            ),
            maxCharacters: Type.Optional(
               Type.Integer({
                  ...
                  maximum: 50000,
                  description:
                     "Maximum characters in returned content. Default 12000.",
               }),
            ),
         ...
         execute: async (_toolCallId, params, signal) => {
            ...
            const mode = isFetchMode(params.mode ?? "")
               ? params.mode
               : "markdown";
            const maxCharacters =
               typeof params.maxCharacters === "number"
                  ...
                  : 12000;
            const timeoutMs =
               typeof params.timeoutMs === "number" ? params.timeoutMs : 15000;
         ...
         renderResult(result, { expanded, isPartial }, theme) {
            ...
            const details = result.details as
               ...
               | undefined;
            if (!expanded) {
               const host = details?.finalUrl
                  ? summarizeUrlHost(details.finalUrl)
                  : "done";
